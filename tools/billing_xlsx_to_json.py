...
def convert_billing() -> None:
    """Convert billing_codes.xlsx to billing_codes.json."""
    ...
    print(f"Billing: {len(codes)} codes → {json_path}")
    print(f"  search_terms filled: {filled_search}/{len(codes)}")
    for g, n in sorted(groups.items()):
        print(f"  {g}: {n} codes")


def convert_diagnostic() -> None:
    """Convert diagnostic_codes.xlsx to diagnostic_codes.json."""
    ...
    print(f"Diagnostic: {len(codes)} codes → {json_path}")
    print(f"  search_terms filled: {filled_search}/{len(codes)}")
    print(f"  suggested_billing_codes filled: {filled_billing}/{len(codes)}")
