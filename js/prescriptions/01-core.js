...
const CONFIG = {
  ...
  defaultLocation: "Michael Garron Hospital (MGH)",
  storage: {
...
class LocationManager {
  ...
  async loadBaseLocations() {
    try {
      const data = typeof LOCATION_DATA !== "undefined" ? LOCATION_DATA : null;
      if (data && Array.isArray(data.locations) && data.locations.length > 0) {
        BASE_LOCATIONS = data.locations;
        this.locationsLoaded = true;
      } else {
        console.warn("Location data is empty or missing, using fallback");
...
class ProviderManager {
  ...
  async loadAuthorizedProviders() {
    try {
      const data = typeof PROVIDER_DATA !== "undefined" ? PROVIDER_DATA : null;
      if (Array.isArray(data)) {
        this.authorizedProviders = data;
      }
    } catch (error) {
      console.warn("Failed to load provider data, using hardcoded fallback:", error);
...
class DataLoader {
  async loadMedications() {
    try {
      const data = typeof PRESCRIPTION_DATA !== "undefined" ? PRESCRIPTION_DATA : null;
      if (data && Array.isArray(data.meds)) {
        return data.meds;
      }
      console.error("Prescription data is missing or invalid");
      return [];
    } catch (error) {
